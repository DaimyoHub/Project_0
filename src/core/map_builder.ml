...
type data = (level Array.t) Array.t
...
let make_flat_map size =
  {
    size = size;
    data = Array.make_matrix size.x size.y Top
  }
 ...
 *)
let up_on_range map height x x_length y y_length =
  ...
  for i = x to x + x_length - 1 do
    for j = y to y + y_length - 1 do
      map.data.(i).(j) <- up_level_by_height map.data.(i).(j) height
    done
  done;
  map
